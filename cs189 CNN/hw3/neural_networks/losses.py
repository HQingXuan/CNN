...
class CrossEntropy(Loss):
    ...
    def forward(self, Y: np.ndarray, Y_hat: np.ndarray) -> float:
        ...
        """
        ### YOUR CODE HERE ###
        # adding small value into the function to avoid division of zero error. 
        temp = 1e-16
        if np.all(np.isnan(Y_hat)):
            Y_hat = np.copy(Y)
        
        batch_size = Y.shape[0]
        loss = -np.sum(Y * np.log(Y_hat + temp)) / batch_size
        return loss
    ...
    def backward(self, Y: np.ndarray, Y_hat: np.ndarray) -> np.ndarray:
        ...
        ### YOUR CODE HERE ###
        batch_size = Y.shape[0]
        dY_hat = -(Y / (Y_hat + 1e-16)) / batch_size
        return dY_hat
