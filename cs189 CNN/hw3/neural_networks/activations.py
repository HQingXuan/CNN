...
class SoftMax(Activation):
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        for i in range(batch_size):
            softmax_i = softmax_output[i, :]
            softmax_i = softmax_i.reshape(-1, 1)
            jacobian_matrix = np.diagflat(softmax_i) - np.dot(softmax_i, softmax_i.T)
            dZ[i, :] = np.dot(dY[i, :], jacobian_matrix)
